...
/** ==========================
 * Small reusable tilt/shine card
 * =========================== */
function TiltImage({ img, onOpen, index, className = "" }) {
...
/** ==========================
 * Split layout with image (left or right) + rich text (enhanced to open lightbox)
 * =========================== */
...
export default function Story() {
    ...
    const pairedBlocks = [
        ...
        },
        {
            ...
            // caption: "Premium worktop with gold fixtures",
        },
    ];
    ...
    // Helper to find index of an image by src for lightbox
    ...
    return (
        <section className="relative bg-black text-white">
            ...
            <div className="mx-auto max-w-6xl space-y-14 px-6 py-16 md:py-20">
                {/* Pair each paragraph with its own image (alternating) */}
                {pairedWithImages.map((b, i) => (
                    <SectionSplit
                        ...
                        caption={b.caption}
                        reverse={i % 2 === 1}
                        onOpen={(/*idx*/) => setOpenIndex(indexBySrc(b.img.src))}
                        index={i}
                    >
                        <p>{b.copy}</p>
                    </SectionSplit>
                ...
                {/* Feature rows with unique paired images (no duplicates) */}
                {featuresWithImages.map((f, i) => (
                    <div key={`f-${i}`} className="grid items-center gap-6 md:grid-cols-12">
                        <div className={`md:col-span-5 ${i % 2 ? 'md:order-2' : ''}`}>
                            <TiltImage img={f.img} index={i + 4} onOpen={() => setOpenIndex(indexBySrc(f.img.src))} />
                        </div>
                        <div className={`md:col-span-7 ${i % 2 ? 'md:order-1' : ''}`}>
                            <Section kicker={f.kicker} copy={f.copy} />
                        </div>
                    </div>
                ))}

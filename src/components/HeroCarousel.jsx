...
/**
 * HeroCarousel — smooth, accessible, no external libs
 * - Full-bleed images
 * - Left-side overlay (kicker, title, subtitle, CTA)
 * - Auto-play with pause on hover/focus, arrows, dots, swipe
 */
        ...
        >
            ...
            >
                            ...
                            <div className="relative z-10 flex h-full items-center">
                                <div className="mx-auto w-full max-w-7xl px-6">
                                    <div className="max-w-lg">
                                        ...
                                        {s.subtitle && (
                                            <p className="mt-3 text-sm text-white/80 md:text-base">{s.subtitle}</p>
                                        )}
                                        {s.cta && (
                                            <a
                                                href={s.href || "#"}
                ...
                <button
                    ...
                    onClick={() => go(index - 1)}
                    className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full border border-white/20 bg-black/30 p-2 text-white backdrop-blur transition hover:bg-black/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C1A88B]/60"
                >
                    ‹
                </button>
                <button
                    ...
                    onClick={() => go(index + 1)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full border border-white/20 bg-black/30 p-2 text-white backdrop-blur transition hover:bg-black/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C1A88B]/60"
                >
                    ›
                </button>
                ...
                <div className="absolute bottom-4 left-6 flex gap-2">
                    {slides.map((_, i) => (
                        <button
                            ...
                            onClick={() => go(i)}
                            className={`h-2.5 w-2.5 rounded-full transition ${i === index ? "bg-[#C1A88B]" : "bg-white/40 hover:bg-white/70"
                                }`}
                        />
                    ))}
                </div>
